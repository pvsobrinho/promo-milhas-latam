...
while tentativas < NUM_ITERACOES:
    ...
    # Salva o mini-print com o mesmo padrão da iteração (para conferir se a região está correta)
...
if milhas_extraidas:
    linhas = [
        "Pomo Milhas – processamento concluído",
        f"Rota: {ORIGEM} → {DESTINO}",
        f"Datas processadas: {len(milhas_extraidas)}",
        "",
        "Top 3 menores preços:",
    ]
    for i, (data, milhas, horario) in enumerate(top_3, 1):
        linhas.append(f"  {i}. {data} às {horario}: {milhas} milhas")
    corpo = "\n".join(linhas)
    enviar_email("✅ Pomo Milhas – processamento concluído", corpo)
else:
    enviar_email(
        "⚠️ Pomo Milhas – sem resultados",
        f"Pomo Milhas ({ORIGEM} → {DESTINO}) finalizou, mas nenhum valor em milhas foi extraído."
    )
